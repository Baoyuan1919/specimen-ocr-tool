...
"""
标本照片 → Excel 自动填表
基于 RapidOCR（ONNX 运行时，轻量免安装深度学习框架）
"""
...
class App:
    ...
    def _build_ui(self):
        ...
        self.log.tag_config("i", foreground="#d4d4d4")
        self.log.tag_config("ok", foreground="#4ec9b0")
        self.log.tag_config("err", foreground="#f44747")
        self.log.tag_config("w", foreground="#ce9178")
        self.log.tag_config("b", foreground="#569cd6", font=('Consolas',11,'bold'))
        ...
        self.run_btn = ttk.Button(bf, text="🚀 开始识别并填表", width=24,
                                   command=self._start)
        self.run_btn.pack(side=tk.RIGHT, padx=4)
        ttk.Button(bf, text="清空日志", command=self._clr_log).pack(side=tk.RIGHT, padx=4)
    ...
    def _refresh(self):
        ...
        if d and os.path.isdir(d):
            ...
            self.fs.config(text=f"已选（{n} 张图片）", foreground="green")
        else:
            self.fs.config(text="（未选择）", foreground="gray")
        if e and os.path.isfile(e):
            self.es.config(text="✅ 文件存在", foreground="green")
        elif e:
            self.es.config(text="⚠️ 不存在，处理时将自动创建", foreground="orange")
        else:
            self.es.config(text="（未选择）", foreground="gray")

    def _pick_folder(self):
        d = filedialog.askdirectory(title="选择标本照片文件夹")
        if d: self.image_folder.set(d)

    def _pick_excel(self):
        p = filedialog.askopenfilename(title="选择 Excel", filetypes=[("Excel","*.xlsx")])
        if p: self.excel_path.set(p)

    def _new_excel(self):
        p = filedialog.asksaveasfilename(title="保存 Excel", defaultextension=".xlsx",
                                          filetypes=[("Excel","*.xlsx")])
        if p: create_excel(p); self.excel_path.set(p); self._wl(f"📋 已创建: {Path(p).name}", "ok")

    def _open_dir(self):
        p = self.excel_path.get()
        if p and os.path.isfile(p): os.startfile(os.path.dirname(p))
    ...
    def _start(self):
        ...
        d = self.image_folder.get(); e = self.excel_path.get()
        ...
        def worker():
            def pc(cur, tot):
                self.root.after(0, lambda: self.pb.configure(value=cur, maximum=tot))
                self.root.after(0, lambda: self.pl.config(text=f"处理中 {cur}/{tot} [{cur*100//tot}%]"))
            def lc(msg, tag="i"):
                self.root.after(0, lambda m=msg, t=tag: self._wl(m, t))
            try:
                process_folder(d, e, pc, lc)
            except Exception as ex:
                self.root.after(0, lambda: self._wl(f"❌ {ex}", "err"))
            finally:
                self.running = False
    ...
    def _save_config(self):
        try:
            ...
            with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
                json.dump({"image_folder": self.image_folder.get(),
                           "excel_path": self.excel_path.get()}, f)
        except: pass
